...
extension UIViewController {
    ...
    func k_navigationItem(position: kUIBarItemPositionType = .right, title: String, style: UIBarButtonItemStyle = .plain, block: (()->Void)?)  {
        ...
        if position == .right {
            ...
            self.navigationItem.rightBarButtonItem = UIBarButtonItem.init(title: title, style: style, target: self, action: #selector(k_rightItemAction))
            
        } else {
            
            objc_setAssociatedObject(self, &kUIViewControllerLeftTitleKey, block, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
            
            self.navigationItem.leftBarButtonItem = UIBarButtonItem.init(title: title, style: style, target: self, action: #selector(k_leftItemAction))
        }
    }
...
extension UIViewController {
    ...
    func k_navigationItemWith(position: kUIBarItemPositionType = .right, image: UIImage, style: UIBarButtonItemStyle = .plain, block: (()->Void)?)  {
        ...
        if position == .right {
            ...
            self.navigationItem.rightBarButtonItem = UIBarButtonItem.init(image: image, style: style, target: self, action: #selector(k_rightImgItemAction))

        } else {
            
            objc_setAssociatedObject(self, &kUIViewControllerLeftImageKey, block, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)

            self.navigationItem.rightBarButtonItem = UIBarButtonItem.init(image: image, style: style, target: self, action: #selector(k_leftImgItemAction))
        }
    }
